...
- `name` (string): If provided, it allows the component to be easily modified without having to know or remember its index using `require('feline').components[component_name]`. If that's not needed, then the component name can be omitted without any issues.
